...
def test_une_correction_haute_confiance_est_appliquee():
    ...
    new_text, findings, entities = fc.apply_verdicts(text, [v])
    body = new_text.split("\n\n")[0]
...
def test_une_correction_confiance_basse_n_est_pas_appliquee():
    text = "Le rapport Charpin de 2024 est une référence."
    v = verdict(
        "rapport Charpin de 2024",
        verdict="corrige",
    ...
    assert "rapport Charpin de 2011" not in new_text
    assert len(findings) == 1
...
def test_horodatage_derive_de_la_position_et_de_la_duree():
    ...
    _, findings, _ = fc.apply_verdicts(text, [v], duration=1000.0)
